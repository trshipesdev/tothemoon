...
- [ ] **Live transaction latency** — CRITICAL before live trading. Two separate issues:
  (1) **Solana**: `_sol_submit_tx` returns a signature immediately but does NOT wait for confirmation. The bot records the quote price as the fill price, but the actual on-chain execution happens 400ms–1s later when price may have moved further. If the tx fails to land (Solana congestion), the bot thinks it sold but still holds the position. Need to poll for confirmation or use `preflight_commitment`.
  (2) **EVM (Base/ETH/BSC/Poly)**: `wait_for_transaction_receipt(timeout=120)` BLOCKS the entire engine loop for up to 2 minutes per sell. While one EVM sell is confirming, none of your other positions are being monitored. Fix: run live sells in a background thread, flag the position as "sell_pending", and skip position checks for it until the tx resolves.
